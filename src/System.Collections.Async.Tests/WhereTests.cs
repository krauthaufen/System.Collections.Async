...
namespace System.Collections.Async.Tests
{
    ...
    public class WhereTests
    {
        ...
        public void EmptyWhereNone()
        {
            ...
            var rs = xs.Where(x => false, CancellationToken.None).ToArray(CancellationToken.None).Result;
        ...
        {
            var xs = AsyncEnumerable.Empty<int>();
            var rs = xs.Where(x => true, CancellationToken.None).ToArray(CancellationToken.None).Result;
            Assert.IsTrue(rs.Length == 0);
        }
        ...
        {
            var xs = AsyncEnumerable.FromValues(new[] { 10, 20, 30 });
            var rs = xs.Where(x => x == 30, CancellationToken.None).ToArray(CancellationToken.None).Result;
        ...
        public void MultipleElementsWhereMultiple()
        {
            ...
            var rs = xs.Where(x => x % 10 == 0, CancellationToken.None).ToArray(CancellationToken.None).Result;
    ...
    public class WhereWithIndexTests
    {
        ...
        {
            var xs = AsyncEnumerable.Empty<int>();
            var rs = xs.Where((x, i) => false, CancellationToken.None).ToArray(CancellationToken.None).Result;
            Assert.IsTrue(rs.Length == 0);
        }
        ...
        {
            var xs = AsyncEnumerable.Empty<int>();
            var rs = xs.Where((x, i) => true, CancellationToken.None).ToArray(CancellationToken.None).Result;
            Assert.IsTrue(rs.Length == 0);
        }
        ...
        {
            var xs = AsyncEnumerable.FromValues(new[] { 10, 20, 30 });
            var rs = xs.Where((x, i) => i == 2, CancellationToken.None).ToArray(CancellationToken.None).Result;
            Assert.IsTrue(rs.Length == 1 && rs[0] == 30);
        }
        ...
        {
            var xs = AsyncEnumerable.FromValues(new[] { 10, 20, 33, 44, 50 });
            var rs = xs.Where((x, i) => i != 2 && i != 3, CancellationToken.None).ToArray(CancellationToken.None).Result;
            Assert.IsTrue(rs.Length == 3 && rs[0] == 10 && rs[1] == 20 && rs[2] == 50);
        }
